...
1: 		halt
		...
		ld a,7
		cp l
		jr nc,1b ; hold untill seq == 1, exit as soon as we go to seq = 2
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

        call goodendcls

        ld      hl,0x1800+8*32+16
        ld      de,goodendtext8
        ld      b,2
        ld      a,16
        call    _printtext
...
1: 		halt
		...
		ld a,20
		cp l
		jr nc,1b ; hold untill seq == 12, exit as soon as we go to seq = 1
...
1: 		halt
		...
		ld a,24
		cp l
		jr nc,1b ; hold untill seq == 2, exit as soon as we go to seq = 3
...
;goodendtext18:
;    db 127+127,127+127,'A'+127,'N'+127,'D'+127,127+127,'R'+127,'E'+127,'M'+127,'E'+127,'M'+127,'B'+127,'E'+127,'R'+127,127+127,127+127
