...
---------------------------------------------------------------
 RAW image converter (24 bpp, uncompressed) to TMS9918 format
---------------------------------------------------------------
Overview
---------------------------------------------------------------
...
// Vars
 ...
 char *name;
 short image[8+2][8192+2][3],header[18],palette[16][3];
...
{ 255,255,255}            // F White         255  255  255
	...
	if ((file=fopen("palette.dat","rb"))==NULL)	{
		printf("Cannot open palette.dat file, using standard TMS colors\n");
	}
	else	{
		for (i=0;i<16;i++)
			for (k=0;k<3;k++)
...
// Test if only one command-line parameter is available
 ...
 if (argc==1)
 {
  printf("Syntax: TMSopt [file.raw]\n");
  printf("raw file header: \n db 72 \n dw maxX (locked to 8) \n dw maxY \n db r,g,b \n db r,g,b \n ...");
  return;
 }
...
// Calculate size
header[12]=fgetc(file);
header[13]=fgetc(file);
header[14]=fgetc(file);
header[15]=fgetc(file);
...
// Load image data
 ...
 for (y=0;y<MAXY;y++)
  for (x=0;x<MAXX;x++)
   for (k=0;k<3;k++)
    image[x+1][y+1][k]=((short)fgetc(file))*scale;        // Scale image
              ...
              else
              {
			   ...
			   if (!NoDither)	{
				// Spread the quantization error
				  ...
				  if (j!=7)   image[xx+1][yy+1][k] = clamp(image[xx+1][yy+1][k])+ quant_error; // 1 *
				  quant_error += q2 ;
				  if (j!=7) image[xx-1][yy+1][k] = clamp(image[xx-1][yy+1][k])+ quant_error; // 3 *
				  quant_error += q2 ;
				  if (j!=7) image[xx+0][yy+1][k] = clamp(image[xx+0][yy+1][k])+ quant_error; // 5 *
				  quant_error += q2 ;
...
txtCHR = fopen("pgtdata.asm","w");
txtCLR = fopen("pctdata.asm","w");
...
headerdata[12]=header[12];
headerdata[13]=header[13];
headerdata[14]=header[14];
headerdata[15]=header[15];


 for (i=0;i<18;i++) fputc(headerdata[i],file);
...
// Prompt elapsed time
 ...
 file=fopen("outdata.raw","wb");

 for (y=0;y<MAXY;y++)
  for (x=0;x<MAXX;x++)
   for (k=0;k<3;k++)
    fputc(inrange8(image[1+x][1+y][k]/scale),file);
