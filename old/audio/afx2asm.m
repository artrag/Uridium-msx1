...
%    data = data(101:end);	% all bytes are usefull
    num = size(data,1);
    ...
    if compr~=' '
     ...
     name = [ 'music' dec2hex(i-1) '.cmp'];
     fid = fopen(name ,'wb');
     fwrite(fid,data,'uint8');
     fclose(fid);
    end
    
    name = [ 'sfx' dec2hex(i-1) '.asm'];
    fid = fopen(name ,'wb');
 
    if compr~=' ' 	
        fwrite(fid,['   ;  ' x(i).name ', without 100 bytes of header, compressed by ' compressor 13 10 ],'char');
%         fwrite(fid,'   dw ' ,'char');
